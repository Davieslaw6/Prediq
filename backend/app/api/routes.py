...
from app.services.provider import MSportProvider
...
async def prediction(match_id:int,db:Session=Depends(get_db)):
    m=db.get(Match,match_id)
    if not m: raise HTTPException(404,"Match not found")
    msport_odds=None
    msport_error=None
    if settings.msport_enabled:
        try: msport_odds=await MSportProvider().find_match_odds(m.home_team,m.away_team,m.kickoff)
        except Exception as exc: msport_error=str(exc)
    try:p=Predictor().predict(db,m,msport_odds=msport_odds)
